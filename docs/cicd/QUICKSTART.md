# AutoGeo CI/CD 快速指南

> 快速了解和使用 AutoGeo 的 CI/CD 流程

---

## 📋 目录

1. [CI 原理对比](#ci-原理对比)
2. [本地 CI 测试](#本地-ci-测试)
3. [GitHub Actions CI](#github-actions-ci)
4. [Electron 打包原理](#electron-打包原理)
5. [常见问题](#常见问题)

---

## CI 原理对比

### 后端 CI (FastAPI + Python)

```
代码推送 → Lint (Ruff) → 类型检查 (MyPy) → 单元测试 (Pytest) → Docker 构建 → 部署
```

**为什么简单？**
- ✅ Python 是解释型语言，不需要编译
- ✅ 依赖管理简单 (uv/pip)
- ✅ 单一运行环境 (Linux)
- ✅ Docker 标准化

### 前端 CI (Electron + Vue3)

```
代码推送 → ESLint → TypeScript → Vitest → Vite Build → Electron 打包 → 发布
         ↓                    ↓                    ↓
      快速检查            Web 部分测试         复杂的跨平台打包
```

**为什么复杂？**
- ❌ 需要跨平台构建 (Linux/Windows/macOS 各自只能构建自己平台)
- ❌ 需要原生依赖 (node_modules 有很多原生模块)
- ❌ Electron 文件巨大 (~200MB+)，构建时间长
- ❌ 代码签名需要证书

---

## 本地 CI 测试

### 运行本地 CI

```bash
# 进入前端目录
cd fronted

# 运行完整 CI
node scripts/ci.js

# 只运行前端
node scripts/ci.js --frontend

# 只运行后端
node scripts/ci.js --backend

# 跳过测试
node scripts/ci.js --skip-test

# 详细输出
node scripts/ci.js --verbose
```

### CI 输出示例

```
╔════════════════════════════════════════════════════════════╗
║         AutoGeo 本地自动化 CI 脚本                       ║
╚════════════════════════════════════════════════════════════╝

ℹ 配置: { frontend: true, backend: true, ... }

▶ 开始前端 CI 检查...
ℹ 运行 ESLint...
✓ ESLint 检查通过
ℹ 运行 TypeScript 类型检查...
✓ TypeScript 检查通过
ℹ 运行单元测试...
✓ 单元测试通过
ℹ 构建前端...
✓ 前端构建成功
✓ 前端 CI 完成！用时: 45.23s

============================================================
  CI 报告
============================================================

前端:
  ✓ 通过: 4
  ✗ 失败: 0
  ⊘ 跳过: 0

  步骤详情:
    ✓ ESLint (5.23s)
    ✓ TypeScript (8.12s)
    ✓ Unit Tests (12.45s)
    ✓ Build (19.43s)

============================================================
总计: ✓ 4 | ✗ 0 | ⊘ 0

🎉 所有检查通过！可以安全推送代码了！
```

---

## GitHub Actions CI

### 触发 CI

```bash
# 推送代码触发 CI
git push origin master

# 创建 PR 触发 CI
gh pr create --title "新功能" --body "描述"

# 打标签触发发布
git tag v2.4.0
git push origin v2.4.0
```

### CI 工作流

| 文件 | 触发条件 | 内容 |
|------|---------|------|
| `frontend-ci.yml` | push/PR to fronted/** | Lint + Test + Build |
| `frontend-release.yml` | push tag v\*.\*.\* | 跨平台打包 + 发布 |
| `backend-ci.yml` | push/PR to backend/** | Lint + Test + 安全扫描 |
| `backend-deploy.yml` | push to master | Docker 构建 + 部署 |
| `dependency-review.yml` | PR to master/ dev | 依赖审查 |

### 如何禁用 Workflows

#### 方法 1：重命名文件（最简单）

```bash
# 在 workflow 文件名前面加下划线，GitHub 就不会识别
cd .github/workflows
mv frontend-ci.yml _frontend-ci.yml.disabled
mv backend-ci.yml _backend-ci.yml.disabled
```

#### 方法 2：改为仅手动触发

修改 workflow 文件，把 `on: push` 改为 `on: workflow_dispatch`：

```yaml
# 原来自动触发：
# on:
#   push:
#     branches: ['**']

# 改为手动触发：
on:
  workflow_dispatch:  # 只能在 GitHub 页面手动运行
```

手动运行位置：`Actions` → 选择 workflow → 点击 `Run workflow`

#### 方法 3：添加禁用条件

在 workflow 文件顶部添加 `if: false`：

```yaml
name: Frontend CI

on:
  push:
    branches: ['**']

# 添加这行来禁用整个 workflow
if: false

jobs:
  # ...
```

#### 方法 4：仓库设置中禁用（不推荐，会禁用所有）

`Settings` → `Actions` → `General` → `Disable all workflows`

---

## Electron 打包原理

### 打包流程

```
┌─────────────────────────────────────────────────────────────┐
│  1. Vite Build (渲染进程)                                    │
│     pnpm run build:renderer                                 │
│     → 生成 dist/ 目录 (HTML/JS/CSS)                         │
├─────────────────────────────────────────────────────────────┤
│  2. Electron 编译 (主进程)                                   │
│     pnpm run build:electron                                 │
│     → 生成 electron/main/*.js                              │
├─────────────────────────────────────────────────────────────┤
│  3. electron-builder 打包                                    │
│     pnpm run dist                                           │
│     ↓                                                       │
│  ┌─────────────────────────────────────────────────────────┐ │
│  │  a) 下载 Electron 二进制 (~200MB)                       │ │
│  │  b) 打包 Vite 构建产物                                   │ │
│  │  c) 打包主进程代码                                       │ │
│  │  d) 生成安装包                                           │ │
│  └─────────────────────────────────────────────────────────┘ │
│     ↓                                                       │
│  ┌─────────┬─────────┬─────────┐                           │
│  │ Windows │  macOS  │  Linux  │                           │
│  │ .exe    │  .dmg   │.AppImage│                           │
│  │ .msi    │  .zip   │ .deb    │                           │
│  │         │         │ .rpm    │                           │
│  └─────────┴─────────┴─────────┘                           │
└─────────────────────────────────────────────────────────────┘
```

### 跨平台构建策略

```yaml
# 每个平台只能构建自己的包！
strategy:
  matrix:
    include:
      - os: ubuntu-latest
        platform: linux
        target: [AppImage, deb, rpm]
      - os: windows-latest
        platform: windows
        target: [nsis, portable]
      - os: macos-latest
        platform: macos
        target: [dmg, zip]
```

---

## 常见问题

### Q1: 为什么 Electron CI 这么慢？

**A:** Electron CI 慢的原因：

1. **下载 Electron 二进制** (~200MB，每次都要下载)
2. **跨平台构建** 需要在 3 个 OS 上分别运行
3. **Playwright 浏览器** 需要单独下载 Chromium
4. **原生模块编译** node-gyp 编译需要时间

**优化方案：**
- 使用 GitHub Actions 缓存
- 使用 electron-builder 的预构建二进制
- 考虑使用 [electron-builder.com](https://www.electron.build/) 云服务

### Q2: 代码签名怎么做？

**Windows:**
```bash
# 购买代码签名证书 (DigiCert, Sectigo 等)
# 配置 GitHub Secrets:
WIN_CSC_LINK=your-certificate-file
WIN_CSC_KEY_PASSWORD=your-password
```

**macOS:**
```bash
# 需要苹果开发者账号 ($99/年)
# 配置 GitHub Secrets:
CSC_LINK=your-certificate-file
CSC_KEY_PASSWORD=your-password
APPLE_ID=your-apple-id
APPLE_ID_PASSWORD=app-specific-password
APPLE_TEAM_ID=your-team-id
```

### Q3: 如何在本地测试 Electron 打包？

```bash
# 进入前端目录
cd fronted

# 安装依赖
npm install

# 构建
npm run build

# 打包 (当前平台)
npm run dist

# 只打包 Windows (在 Windows 上)
npm run dist:win

# 只打包 macOS (在 macOS 上)
npm run dist:mac

# 只打包 Linux (在 Linux 上)
npm run dist:linux
```

### Q4: CI 失败了怎么办？

```bash
# 1. 先在本地跑 CI
node scripts/ci.js --verbose

# 2. 查看 GitHub Actions 日志
gh run view [run-id]

# 3. 本地重现问题
# (按照 CI 步骤手动执行)

# 4. 修复后重新推送
git add .
git commit -m "fix: 修复 CI 问题"
git push
```

---

## 文件清单

| 文件 | 说明 |
|------|------|
| `.github/workflows/frontend-ci.yml` | 前端 CI |
| `.github/workflows/frontend-release.yml` | 前端发布 |
| `.github/workflows/backend-ci.yml` | 后端 CI |
| `.github/workflows/backend-deploy.yml` | 后端部署 |
| `fronted/scripts/ci.js` | 本地 CI 脚本 |
| `fronted/electron-builder.yml` | Electron 打包配置 |
| `fronted/scripts/before-build.js` | 构建前脚本 |
| `fronted/scripts/after-pack.js` | 打包后脚本 |

---

**最后更新**: 2026-02-11
**维护者**: 老王
